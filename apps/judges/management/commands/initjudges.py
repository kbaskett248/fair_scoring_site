...
from apps.judges.models import JudgeEducation, JudgeFairExperience
...
class Command(BaseCommand):
    ...
    def init_experience(self, experiences):
        ...
        for short_desc, long_desc in experiences:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Successfully created Experience "%s"' % exp)
                )
            else:
                self.stdout.write(
                    self.style.NOTICE('Experience "%s" already exists' % exp)
                )
    ...
    def init_education(self, educations):
        ...
        for short_desc, long_desc in educations:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Successfully created Education "%s"' % educat)
                )
            else:
                self.stdout.write(
                    self.style.NOTICE('Education "%s" already exists' % educat)
                )
