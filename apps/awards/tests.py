...
class AwardTests(TestCase):
    ...
    def setUp(self):
        self.award1 = make_Award(name="Award 1", award_order=1, award_count=1)
        make_AwardRule(
            award=self.award1, trait="trait_a", operator_name="IS", value="A"
        )
        make_AwardRule(
            award=self.award1, trait="trait_b", operator_name="IS", value="A"
        )
        self.award2 = make_Award(
            name="Award 2", award_order=2, award_count=1, exclude_awards=[self.award1]
        )
        make_AwardRule(
            award=self.award2, trait="trait_a", operator_name="IS", value="B"
        )
        make_AwardRule(
            award=self.award2, trait="trait_b", operator_name="IS", value="B"
        )
...
class AwardRuleTests(HypTestCase):
    ...
    def test_award_rule_str(self):
        ...
        self.assertEqual(
            str(rule),
            "{trait} {0} {value}".format(rule.operator.display, **rule.__dict__),
        )
    ...
    def generic_operator_test(
        self, operator_name, rule_value, instance_value, expected_result, test=None
    ):
        TestInstance = namedtuple("TestInstance", ("test_trait"))
        if not test:
            test = self.assertIs
        rule = make_AwardRule(
            trait="test_trait", value=rule_value, operator_name=operator_name
        )
    ...
    def test_allow_instance_Greater_string(self, rule_value: str, instance_value):
        ...
        self.generic_operator_test(
            Greater.internal, rule_value, instance_value, test_value
        )
        self.generic_operator_test(Greater.internal, rule_value, rule_value, False)
        if not isinstance(instance_value, bool):
            self.generic_operator_test(
                Greater.internal, str(instance_value), instance_value, False
            )

    @given(
        numeric_strings(min_value=-40000, max_value=40000),
        floats(min_value=-40000, max_value=40000),
    )
    def test_allow_instance_Greater_numeric(
        self, rule_value: str, instance_value: float
    ):
        self.generic_operator_test(
            Greater.internal,
            rule_value,
            instance_value,
            instance_value > float(rule_value),
        )
        self.generic_operator_test(Greater.internal, rule_value, rule_value, False)
        self.generic_operator_test(
            Greater.internal, str(instance_value), instance_value, False
    ...
    def test_allow_instance_Less_string(self, rule_value: str, instance_value):
        ...
        self.generic_operator_test(
            Less.internal, rule_value, instance_value, test_value
        )
        self.generic_operator_test(Less.internal, rule_value, rule_value, False)
        if not isinstance(instance_value, bool):
            self.generic_operator_test(
                Less.internal, str(instance_value), instance_value, False
            )
    ...
    def test_allow_instance_In(self, rule_value: list, instance_value):
        ...
        formatted_rule_value = ",".join(rule_value)
        self.generic_operator_test(
            In.internal, formatted_rule_value, instance_value, True
        )
    ...
    def test_allow_instance_NotIn(self, rule_value: list, instance_value):
        ...
        formatted_rule_value = ",".join(rule_value)
        self.generic_operator_test(
            NotIn.internal, formatted_rule_value, instance_value, False
        )
