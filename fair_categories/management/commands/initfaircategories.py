...
ETHNICITIES = ["Hispanic", "Black", "Caucasian"]
...
class Command(BaseCommand):
    ...
    def init_categories(self, categories):
        ...
        for desc, subcats in categories:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Successfully created Category "%s"' % cat)
                )
            else:
                self.stdout.write(
                    self.style.NOTICE('Category "%s" already exists' % cat)
                )
            ...
            for abbrev, scdesc in subcats:
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(
                            '\tSuccessfully created Subcategory "%s"' % subcat
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.NOTICE('\tSubcategory "%s" already exists' % subcat)
                    )
    ...
    def init_divisions(self, divisions):
        ...
        for desc in divisions:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Successfully created Division "%s"' % div)
                )
            else:
                self.stdout.write(
                    self.style.NOTICE('Division "%s" already exists' % div)
                )
    ...
    def init_ethnicities(self, ethnicities):
        ...
        for desc in ethnicities:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Successfully created Ethnicity "%s"' % eth)
                )
            else:
                self.stdout.write(
                    self.style.NOTICE('Ethnicity "%s" already exists' % eth)
                )
